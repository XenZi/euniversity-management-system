...
const ConfirmExtendStatus = () => {
    const [loadedApplications,setLoadedApplications] = useState<ExtendStatusApplication[]>([]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if(selectedUser){
            axiosInstance
                .put(`/university/student/status1/${selectedUser.personalIdentificationNumber}`)
                .then((res) => {
                ...
                .catch((err) => {
                    ...
                    toast.error("Something went wrong! Student doesn't have a medical exam!")
                })
        }
    }
    ...
    const handleUserChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const exam = loadedApplications.find((application) => application.id === selectedId);
                ...
                >
                    ...
                    {loadedApplications?.map((application) => (
                        <option value={application.id} key={application.id}>
                            {application.citizen.fullName} - {application.citizen.personalIdentificationNumber}
                        </option>
                    ))}
                </select>
                <button
