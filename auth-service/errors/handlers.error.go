	...
	if writeErr, ok := err.(mongo.WriteException); ok {
		for _, writeError := range writeErr.WriteErrors {
			if writeError.Code == 11000 {
				...
				} else if strings.Contains(writeError.Message, "personalIdentificationNumber_1") {
					return fmt.Errorf("Duplicate entity with personal identification number %s already exists", user.PersonalIdentificationNumber), 422
				}
			}
		}
	}
